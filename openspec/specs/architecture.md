...
See `prisma/schema.prisma`. The single `Event` model has a `VECTOR(768)` column declared as `Unsupported("VECTOR(768)")` so Prisma can migrate the table while keeping the column out of the generated client surface (we read/write it via the SQL files only).
